  ...
  for (idx=0; idx<N_AXIS; idx++) {
    ...
    // NOTE: Computes true distance from converted step values.
      ...
      if (idx == A_MOTOR) {
    	...
    	delta_mm_signed = (corexy_dir_invert_X*(target_steps[X_AXIS]-position_steps[X_AXIS]) + corexy_dir_invert_Y*(target_steps[Y_AXIS]-position_steps[Y_AXIS]))/settings.steps_per_mm[idx];
      } else if (idx == B_MOTOR) {
        delta_mm = (target_steps[X_AXIS]-position_steps[X_AXIS] - target_steps[Y_AXIS]+position_steps[Y_AXIS])/settings.steps_per_mm[idx];
        delta_mm_signed = (corexy_dir_invert_X*(target_steps[X_AXIS]-position_steps[X_AXIS]) + corexy_dir_invert_Y*(-target_steps[Y_AXIS]+position_steps[Y_AXIS]))/settings.steps_per_mm[idx];
      } else {
        delta_mm = (target_steps[idx] - position_steps[idx])/settings.steps_per_mm[idx];
        delta_mm_signed = (corexy_dir_invert_Z*(target_steps[idx] - position_steps[idx]))/settings.steps_per_mm[idx];
      }

      unit_vec[idx] = delta_mm_signed; // Store unit vector numerator. Denominator computed later.

      // Set direction bits. Bit enabled always means direction is negative.
      if (delta_mm_signed < 0.0 ) { block->direction_bits |= get_direction_pin_mask(idx); }
    #else
      target_steps[idx] = lround(target[idx]*settings.steps_per_mm[idx]);
      block->steps[idx] = labs(target_steps[idx]-position_steps[idx]);
      block->step_event_count = max(block->step_event_count, block->steps[idx]);
...
void plan_sync_position()
{
  ...
  for (idx=0; idx<N_AXIS; idx++) {
     ...
     if (idx==X_AXIS) { 
        ...
        pl.position[idx] = (bit_istrue(settings.dir_invert_mask,bit(Z_AXIS)) ? -1 : 1) * sys_position[idx];
      }
    #else
